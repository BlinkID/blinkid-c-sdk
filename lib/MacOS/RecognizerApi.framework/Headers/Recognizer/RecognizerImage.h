...
enum MBImageOrientation
{
     ...
     */
    IMAGE_ORIENTATION_ROTATED_LEFT_90,
     ...
     * is mounted horizontally (i.e. produced image is in "landscape" orienation).
     */
    IMAGE_ORIENTATION_ROTATED_RIGHT_90,
...
/**
  ...
  @code
    ...
    MBRecognizerErrorStatus status = recognizerImageCreateFromRawImage(&img, image_buffer, image_width, image_height, image_stride, RAW_IMAGE_TYPE_BGRA);
...
/**
  ...
  @code
    ...
    MBRecognizerErrorStatus status = recognizerImageSetNewRawBuffer(img, image_buffer, image_width, image_height, image_stride, RAW_IMAGE_TYPE_BGRA);
    // make sure last parameter correctly describes image format, otherwise a crash will occur later while processing the image. There is no way to
    // detect whether this parameter was given correctly while creating the image, so function may return RECOGNIZER_ERROR_STATUS_SUCCESS even if
    // this parameter is wrong.
